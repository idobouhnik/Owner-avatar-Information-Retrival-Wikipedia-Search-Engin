...
PROJECT_ID = 'task3mapReduce315537936'
...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        self.document_length = defaultdict(list)
        # stores a list of (doc_id,length) it will be useful for retreival during query
        self.normalized_length = defaultdict(list)
        # stores a list of (doc_id,normalized_length) it will be useful for retreival during query
    ...
    def write_a_posting_list(b_w_pl, base_dir, bucket_name=None):
        ...
        with closing(MultiFileWriter(base_dir, bucket_id, bucket_name)) as writer:
            ...
            path = str(Path(base_dir) / f'{bucket_id}_posting_locs.pkl')
            bucket = None if bucket_name is None else get_bucket(bucket_name)
            with _open(path, 'wb', bucket) as f:
